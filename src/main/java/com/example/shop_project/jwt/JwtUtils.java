...
public class JwtUtils implements Filter {
    ...
    public String generateToken(AuthenticationRequest userDetails, CustomerType clientType) {
        Map<String, Object> claims = new HashMap<>();
        return createToken(claims, userDetails.getEmail(), clientType);
    }

    private String createToken(Map<String, Object> claims, String subject , CustomerType clientType) {
    ...
    private boolean validateToken(String jwt) throws ServletException {
        ...
        try {
            ...
            newToken = generateToken(new AuthenticationRequest(claims.getSubject(), null), CustomerType.valueOf(claims.getIssuer()));
            removeToken(jwt);
        } catch (Exception e) {
            return false;
        }
